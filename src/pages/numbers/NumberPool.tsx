...
type SortField = 'number' | 'category' | 'code' | 'group' | 'status' | 'reservationCount';
...
const MAX_RESERVATIONS = 3; // Add this constant at the top with other constants

// Add loading and request tracking
...
// Add new interfaces for status check
...
export function NumberPool({ onNumberSelect, selectedCategory: propSelectedCategory, onCategoryChange }: NumberPoolProps = {}) {
  ...
  // Realtime subscriptions for search-visible docs
  const searchVisibleUnsubsRef = useRef<Map<string, () => void>>(new Map());

  // Visibility guard for team-restricted numbers
  ...
  // Initialize with global manager (persistent across navigation)
  useEffect(() => {
    console.log('📋 Connecting to NumberPoolManager...');
    ...
    return unsubscribe;
  }, [selectedCategory, pageSize, user?.id]);
  ...
  useEffect(() => {
    ...
    const performSearch = async () => {
      ...
      // Remember the page where search started
      numberPoolManager.onSearchStart(debouncedSearchTerm);
      
      // Clear listeners for previous search term
      numberPoolManager.clearSearchListenersForTerm(debouncedSearchTerm);
      
      try {
        // Use global manager for search with pagination
        const result = await numberPoolManager.search(debouncedSearchTerm, searchCurrentPage, pageSize);
        // Avoid race conditions: only apply if term hasn't changed
        if (termAtStart === debouncedSearchTerm) {
          setSearchResults(filterByVisibility(result.data));
          setSearchTotalPages(Math.ceil(result.totalItems / pageSize));
          setSearchTotalItems(result.totalItems);
          setSearchHasNextPage(result.hasNextPage);
          setSearchHasPreviousPage(result.hasPreviousPage);
          
          // Don't update manager state with search results to preserve full page data
          // This ensures reserved numbers calculation always has access to full dataset
        }
      } catch (error) {
  ...
  // Realtime listeners for search results (keep visible searched items live)
  useEffect(() => {
  ...
  }, [debouncedSearchTerm, searchCurrentPage, searchTotalPages, searchTotalItems, searchHasNextPage, searchHasPreviousPage, currentPage, totalPages, totalItems, hasNextPage, hasPreviousPage]);

  useEffect(() => {
    // Use allNumbersForReserved to calculate reserved numbers
    // This ensures reserved numbers are always shown regardless of search state
    const userReservedNumbers = allNumbersForReserved.filter(n => n.status === 'reserved' && n.reservedBy === user?.id);
    
    setReservedNumbers(userReservedNumbers);
    setHasReservation(userReservedNumbers.length >= MAX_RESERVATIONS);
  }, [allNumbersForReserved, user?.id]);
  ...
  const openEditModal = (number: NumberPoolType) => {
    //console.log('Opening edit modal for number:', number);
  ...
  const handleUpdateNumber = async () => {
   // console.log('handleUpdateNumber called', { editingNumber, updatingNumber });
    if (!editingNumber || updatingNumber) {
     // console.log('Early return:', { editingNumber: !!editingNumber, updatingNumber });
      return;
    }
   ...
   // console.log('Form data:', { num, cat, code, group, editPoolStatus });
    ...
    if (num.length !== 10) {
     // console.log('Validation failed: Phone number must be exactly 10 digits', { length: num.length });
    ...
    if (!group) {
     // console.log('Validation failed: Group is required');
      toast.error('Group is required');
      return;
    }
    if (!Array.from(CATEGORIES).includes(cat as any)) {
     // console.log('Validation failed: Invalid category', { cat, categories: CATEGORIES });
      toast.error('Please select a valid category');
      return;
    }
    // Validate passcode for admin/coordinator
    if ((isAdmin() || isCoordinator) && !editPoolPasscode.trim()) {
     // console.log('Validation failed: Passcode is required');
    ...
    if (existingNumber) {
     // console.log('Validation failed: Number already exists');
   ...
   // console.log('All validations passed, proceeding with update...');
    try {
      ...
      //console.log('Updating document with data:', numberData);
     ...
     // console.log('Setting updatingNumber to false');
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      numbers.forEach(number => {
        if (number.claimingExpiresAt) {
          // Ensure claimingExpiresAt is a Date object
          const expiresAt = number.claimingExpiresAt instanceof Date 
            ? number.claimingExpiresAt 
            : new Date(number.claimingExpiresAt);
          
          const timeLeft = Math.max(0, expiresAt.getTime() - now);
          if (timeLeft > 0) {
            newCountdowns[number.id] = timeLeft;
          }
        }
      });
      
      setClaimCountdowns(newCountdowns);
    }, 1000);
  ...
  const computeSorted = useCallback((list: NumberPoolType[]) => {
    const result = [...list];
    if (sortConfig.field === 'number' && sortConfig.direction === 'asc') {
  ...
  async function handleReserve(number: NumberPoolType) {
   // console.log('Reserve button clicked for number:', number);
  ...
  async function confirmSetOpen() {
    ...
    try {
      ...
      console.error('Error setting number to open:', error);
  ...
  async function confirmRelease() {
    ...
    try {
      ...
      if (selectedNumber.claimingAgentId) {
        ...
        // If there's a next claim, start its timer
        if (nextClaim) {
  ...
  const handleClaimTimeout = async (number: NumberPoolType) => {
    try {
      ...
      if (numberDoc.exists()) {
        ...
        const claimingExpiresAt = data.claimingExpiresAt?.toDate();
        
        // Only proceed if the claim has actually expired and the number is still reserved
        if (claimingExpiresAt && claimingExpiresAt instanceof Date && claimingExpiresAt.getTime() <= Date.now() && data.status === 'reserved') {
          // Check if this number is still being claimed by this user
          if (data.claimingAgentId === user?.id && user?.id) {
            const claimQueue = data.claimQueue || [];
            const nextClaim = claimQueue.find((claim: { agentId: string; claimedAt: Date }) => claim.agentId !== data.claimingAgentId);

            if (nextClaim) {
              // There's another claim in the queue, transfer to next agent
              const now = new Date();
              const newExpiresAt = new Date(now.getTime() + CLAIM_TIMEOUT);

              const updateData = {
                status: 'reserved' as NumberStatus,
                reservedBy: data.claimingAgentId,
                reservedAt: serverTimestamp(),
                lastStatusChange: serverTimestamp(),
                // Update claiming agent information for next in queue
                claimingAgentId: nextClaim.agentId,
                ...
                originalReservedAt: null,
                originalExpiresAt: null,
                // Update claim queue and reset claim count
                claimQueue: claimQueue.filter((claim: { agentId: string; claimedAt: Date }) => claim.agentId !== data.claimingAgentId),
                claimCount: 0
              };
              ...
              // Start new timer for next claim
              const timer = setTimeout(() => {
                handleClaimTimeout({
                  ...number,
                  claimingAgentId: nextClaim.agentId,
            ...
            // Send notification to the original agent
            await addDoc(collection(db, 'notifications'), {
     ...
     // console.error('Error updating number status after claim period:', error);
      toast.error('Failed to update number status after claim period');
    }
  };
    ...
    try {
      ...
      if (newStatus === 'open') {
        ...
        if (claimQueue.length > 0) {
          ...
          // If there's a second claim in queue, start their timer
  ...
  const handleStatusCheck = async (number: NumberPoolType) => {
    ...
    try {
     ...
     // console.error('Error requesting status check:', error);
  ...
  const handleChangeToOpen = async (number: NumberPoolType) => {
    ...
    try {
      ...
      //console.error('Error changing number status:', error);
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          <input
                            type="text"
                            value={newPoolGroup}
                            onChange={(e) => setNewPoolGroup((e.target as HTMLInputElement).value)}
                            className="w-full pl-3 pr-10 py-2.5 bg-white border-2 border-gray-200 rounded-lg focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all duration-200 text-gray-900 placeholder-gray-400 text-sm"
            ...
            >
              ...
              >
                ...
                <div className="px-6 py-4">
                  ...
                  {/* Action Buttons */}
                  <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 pt-6 border-t border-gray-200">
                    {/*console.log('Rendering Edit Number action buttons', { updatingNumber, editingNumber: !!editingNumber })}*/}
                    <motion.button
                      type="button"
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
  ...
  return (
    ...
    >
      ...
      <td className="px-6 py-4 whitespace-nowrap text-right relative z-10">
        <div className="flex items-center justify-end space-x-3">
          {number.status === 'open' && (
            ...
            >
                ...
                <Clock className="h-4 w-4 mr-1.5" />
                              ...
                              {reservingNumbers.has(number.id) ? 'Reserving...' : 'Reserve'}
            </motion.button>
          )}
          {(number.status === 'pending_verification' || 
            ...
            number.status === 'follow_up') && 
                            ...
                            !(number.claims || []).some((claim: any) => claim.userId === user?.id && claim.status === 'pending') &&
          ...
          {(number.status === 'pending_verification' || 
            ...
            number.status === 'verified' || 
            number.status === 'follow_up') && 
                            (number.claims || []).some(claim => claim.userId === user?.id && claim.status === 'pending') && (
            <motion.span
        ...
        {/* Enhanced Pagination with Firebase Integration */}
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-1">
                ...
                <motion.button
                  ...
                  className="inline-flex items-center px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-l-lg hover:bg-gray-50 hover:text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
                  title="First Page"
                >
                ...
                <motion.button
                  ...
                  className="inline-flex items-center px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 hover:bg-gray-50 hover:text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
                  title="Previous Page"
                >
                  <ChevronLeft className="h-4 w-4" />
                </motion.button>

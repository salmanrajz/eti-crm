...
export function LeadDetails() {
  ...
  async function handleLeadUpdate(updates: Partial<Lead>) {
    ...
    try {
      ...
      // If lead is verified, only allow updates from admin or manager
      if (leadData.status === 'verified' && !['admin', 'manager'].includes(user.role)) {
      ...
      const nextStatus = isAgentResubmittingFollowUp ? 'pending_verification' : (updates.status || leadData.status);
      const updateData = {
        ...updates,
      ...
      // If agent resubmits from follow_verification OR verifier updated plans, set numbers to pending_verification
      if ((isAgentResubmittingFollowUp || (updates.plans && user.role === 'verifier')) && (updates.plans || leadData.plans)) {
        const plansToUpdate = updates.plans || leadData.plans || [];
        const updatePromises = plansToUpdate.map(async (plan: any) => {
      ...
      toast.success(isAgentResubmittingFollowUp ? 'Lead resubmitted for verification' : 'Lead updated successfully');
      setIsEditing(false);
    } catch (error) {

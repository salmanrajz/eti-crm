...
export function VerifierDashboard({ user }: VerifierDashboardProps) {
  ...
  async function loadVerifierData() {
    try {
      ...
      // Get all leads for metrics
      const allVerificationQuery = query(
        collection(db, 'leads'),
        where('status', 'in', ['pending_verification', 'verified', 'rejected', 'follow_verification'])
      );
      const allVerificationSnapshot = await getDocs(allVerificationQuery);
      let allVerificationData = allVerificationSnapshot.docs.map(doc => ({
        ...doc.data(),
        createdAt: doc.data().createdAt?.toDate?.() || doc.data().createdAt,
      ...
      // Filter leads based on verifier's group assignment for metrics
      if (!hasAllGroups) {
        allVerificationData = allVerificationData.filter(lead => {
          // Check if any of the lead's plans belong to any of the verifier's groups
          const hasMatchingGroup = lead.plans?.some(plan => {
            // Normalize group names for comparison
            const planGroup = plan.group?.toLowerCase();
      ...
      // Filter metrics data for current month
      const currentMonthData = allVerificationData.filter(lead => {
        const leadDate = new Date(lead.createdAt);
        return leadDate >= startOfMonth && leadDate <= endOfMonth;
      });

      // Calculate metrics
      // Pending verification shows ALL pending leads (not filtered by month)
      const metrics = {
        pendingVerificationCount: allVerificationData.filter(l => l.status === 'pending_verification').length,
        verifiedCount: currentMonthData.filter(l => l.status === 'verified').length,
        rejectedCount: currentMonthData.filter(l => l.status === 'rejected').length,
        followUpCount: currentMonthData.filter(l => l.status === 'follow_verification').length
      };
  ...
  const verificationStats = [
    ...
    {
      name: 'Verified Leads',
      value: verificationMetrics.verifiedCount,
      icon: CheckCircle,
    ...
    {
      name: 'Rejected Leads',
      value: verificationMetrics.rejectedCount,
      icon: XCircle,
    ...
    },
    {
      ...
      value: verificationMetrics.followUpCount,
      icon: AlertTriangle,
      color: 'bg-gradient-to-br from-orange-500 to-orange-600',
      textColor: 'text-orange-600',
      status: 'follow_verification'
    }
  ];
      ...
      <div className="bg-white shadow-lg rounded-lg overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">
            {currentStatus === 'pending_verification' && 'Pending Verification Leads'}
            {currentStatus === 'verified' && 'Verified Leads'}
            {currentStatus === 'rejected' && 'Rejected Leads'}
            {currentStatus === 'follow_verification' && 'Follow-up Verification Leads'}
          </h2>
        </div>

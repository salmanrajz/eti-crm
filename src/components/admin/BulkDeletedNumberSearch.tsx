/**
 * ===============================================================================
 * BULK DELETED NUMBER SEARCH COMPONENT - ADMIN TOOL
 * ===============================================================================
 * 
 * This component allows administrators to search for multiple numbers from the
 * deletedNumbers collection at once. Users can paste a list of numbers (one per
 * line or comma-separated) and get detailed information about each deleted number.
 * 
 * FEATURES:
 * 
 ...
 *    - Show deletion metadata (deletedAt, originalId)
 ...
 * administrators to quickly check the status of multiple deleted numbers.
 * ===============================================================================
 */
...
interface BulkDeletedNumberSearchProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export function BulkDeletedNumberSearch({ isOpen, onClose }: BulkDeletedNumberSearchProps) {
  ...
  // Search for numbers in Firebase deletedNumbers collection
  const handleSearch = async () => {
    ...
    try {
      ...
      for (let i = 0; i < numbers.length; i += batchSize) {
        ...
        try {
          // Query Firebase deletedNumbers collection for numbers in this batch
          const q = query(
              ...
              if (doc) {
                ...
                searchResults.push({
                  ...
                  data: {
                    ...
                    visibleToFreelancers: data.visibleToFreelancers,
                    teamVisibility: data.teamVisibility
                  }
                });
              }
            } else {
      ...
      toast.error('Error searching deleted numbers. Please try again.');
    } finally {
      setSearching(false);
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Number', 'Status', 'Category', 'Group', 'Code', 'Plan', 'Deleted At', 'Original ID', 'Found'];
    const rows = results.map(result => [
      ...
      result.data?.deletedAt ? format(result.data.deletedAt, 'yyyy-MM-dd HH:mm:ss') : 'N/A',
      result.data?.originalId || 'N/A',
      result.found ? 'Yes' : 'No'
    ]);
    ...
    a.href = url;
    a.download = `bulk-deleted-number-search-${new Date().toISOString().split('T')[0]}.csv`;
    ...
    const text = results
      .map(result => {
        if (result.found && result.data) {
          return `${result.number},${result.data.status},${result.data.category || 'N/A'},${result.data.group || 'N/A'},${result.data.code || 'N/A'},${result.data.deletedAt ? format(result.data.deletedAt, 'yyyy-MM-dd') : 'N/A'}`;
        }
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Bulk Deleted Number Search</h2>
              <p className="text-sm text-gray-500">Search multiple numbers from deletedNumbers collection</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <div className="mb-6">
            ...
            <textarea
              ...
              className="w-full h-40 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 resize-none font-mono text-sm"
              disabled={searching}
            />
            ...
            >
              {searching ? (
                ...
                <>
                  ...
                  <span>Search Deleted Numbers</span>
                </>
              )}
            </button>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase">Deleted At</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase">Original ID</th>
                    </tr>
                  </thead>
                          ...
                          <td className="px-4 py-3 text-sm text-gray-600">
                            {result.data?.deletedAt ? (
                              <span className="flex items-center gap-1">
                                ...
                                {format(result.data.deletedAt, 'yyyy-MM-dd HH:mm')}
                              </span>
                            ) : '-'}
                          </td>
                          <td className="px-4 py-3 text-sm text-gray-600 font-mono text-xs">
                            {result.data?.originalId || '-'}
                          </td>
                        </motion.tr>
                      ))}
                    </AnimatePresence>

 ...
 */
export const bulkDNCImport = functions
  .region('us-central1')
  .runWith({
  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
      const { records, batchSize = DEFAULT_BATCH_SIZE, source = 'bulk_import', addedBy, dryRun = false }: BulkImportRequest = req.body;
...
async function processBatch(
  batch: DNCImportRecord[],
  source: string,
  addedBy?: string,
  dryRun: boolean = false
): Promise<{ successful: number; failed: number; duplicates: number; errors: string[] }> {
 ...
 */
async function checkExistingNumbers(numbers: string[]): Promise<Set<string>> {
  const db = admin.firestore();
 ...
 */
export const bulkDNCImportStatus = functions
  .region('us-central1')
  .https.onRequest(async (req, res) => {
    // Set CORS headers
    res.set('Access-Control-Allow-Origin', '*');

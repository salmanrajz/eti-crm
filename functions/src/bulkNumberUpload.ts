...
// Valid categories
const VALID_CATEGORIES = ['Platinum', 'Gold', 'Silver', 'Bronze'] as const;
 ...
 */
export const bulkNumberUploadV2 = functions
  .runWith({
...
async function processUploadJob(
  ...
  try {
    ...
    teamsSnap.docs.forEach(doc => {
      const data = doc.data();
      const name = (data?.name || data?.teamName || '').toString().toLowerCase();
      if (name) teamIdByName.set(name, doc.id);

 ...
 */
export const handleReservationExpiry = onDocumentUpdated({
  ...
  try {
    ...
    if (expiresAt && expiresAt <= now) {
      ...
      await processReservationExpiry(numberId);
    }
  } catch (error) {
 ...
 */
export const processReservationExpiry = async (numberId: string) => {
  try {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      const expiresAt = currentData.expiresAt?.toDate();
      if (!expiresAt || expiresAt > now) {
 ...
 */
export const triggerReservationExpiry = onCall({
  region: 'us-central1'
}, async (request) => {
  if (!request.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
      "You must be logged in to trigger reservation expiry"
    );
  }
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      "internal",
      `Failed to trigger expiry: ${error.message}`
    );
  }
});
 ...
 */
export const testReservationExpiry = onCall({
  region: 'us-central1'
}, async (request) => {
  ...
  if (!request.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
      "You must be logged in to test reservation expiry"
    );
  }
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      "internal",
      `Failed to test reservation expiry: ${error.message}`
    );
  }
});
 ...
 */
export const backupReservationExpiry = onSchedule({
  ...
  try {
    ...
    for (const doc of snapshot.docs) {
      await processReservationExpiry(doc.id);
    }

...
export const updateNumberPoolStatsOnCreate = onDocumentCreated({
  ...
  try {
    ...
    const updates: any = {
      ...
      [categoryKey]: FieldValue.increment(1),
    ...
    shardDocs.forEach(doc => {
      ...
      Object.keys(data || {}).forEach(key => {
        if (key.startsWith('totalItems_')) {
          ...
          categoryCounts[category] = (categoryCounts[category] || 0) + (data?.[key] || 0);
        }
      });
    });
 ...
 */
export const autoAggregateStats = functions.pubsub
 ...
 */
export const aggregateStatsShards = functions.https.onCall(async (data, context) => {
  // Auth check
  if (!context.auth) {
 ...
 */
export const initializeNumberPoolStats = functions.https.onCall(async (data, context) => {
  // Auth check
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Auth required');
  }

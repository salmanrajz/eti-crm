/**
 * ===============================================================================
 * USER RESERVED NUMBERS SYNCHRONIZATION (COMPILED VERSION)
 * ===============================================================================
 * 
 * This is the compiled JavaScript version that maintains synchronization between
 * the main numberPool collection and per-user reserved numbers collections.
 * It ensures that reserved numbers are properly tracked for each user.
 * 
 * FUNCTION INCLUDED:
 * 
...
// Import Firebase Functions v2 and Admin SDK modules
...
exports.updateUserReservedNumbersOnWrite = (0, firestore_1.onDocumentWritten)({
    ...
    if (!after && before) {
        ...
        if (prevUid) {
            await db.collection('users').doc(prevUid).collection('reservedNumbers').doc(numberId).delete().catch(() => { });
        }
        return;
    }
    if (!after)
    ...
    if (status === 'reserved' && reservedBy) {
        ...
        const targetRef = db.collection('users').doc(reservedBy).collection('reservedNumbers').doc(numberId);
        await targetRef.set({
            numberId,
            number: after.number,
